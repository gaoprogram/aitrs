  ...
  <div class="content">
    ...
    >
      <div class="content">
        ...
        <div class="item-row">
          <span class="name w-210">调休使用优先级：</span>
          <el-select class="filter-item"
                     v-model="ruleObj.Priority"
          >
            <el-option v-for="item in getTXPriority" :key="item.Id" :label="item.Name" :value="item.Id">
            </el-option>
          </el-select>
        </div>
        <div class="item-row" v-show="ruleObj.Priority === 2">
          <span class="name w-210"></span>
          <div style="display: inline-block">
            <div v-for="item in PSort"
                 :key="item.code"
                 v-dragging="{ item: item, list: PSort, group: 'PSort' }"
                 class="psort_dragging"
            >
              <i class="el-icon-rank" style="font-size: 16px"></i>
              <span style="font-size: 16px;margin-left: 5px">{{item.value}}</span>
            </div>
            <span style="color: #d8dce5;">*可拖拽排序</span>
          </div>
        </div>
        <el-card class="box-card">
          ...
          <div class="text item">
            1.快速工时计算，只验证日期是否为工作日，休假日，具体工时按日小时折算来计算，不核验小时数是否为具体准确的工作时长。<br />
            2.真实工时计算，不只验证日期是否为工作日，休假日，具体工时按员工请假那天的考勤规则对应的具体时长来计算。<br />
            3.一个考勤组只能挂一个规则，如果一旦更新保存，则新保存的规则将覆盖原数据。
          </div>
        </el-card>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      async _getTXPriority () {
        // 调休使用优先级
        await getTXPriority().then(res => {
          if (res.data.State === REQ_OK) {
            this.getTXPriority = res.data.Data
          } else {
            Message.error('调休使用优先级获取失败！')
          }
        }).catch(() => {
          Message.error('调休使用优先级获取失败！')
        })
      },
      async _getDaysOffRulee () {
        // 获取调休规则
        await getDaysOffRule(this.$route.query.ruleCode).then(res => {
          if (res.data.State === REQ_OK) {
            this.ruleObj = res.data.Data
            this.ruleObj.DayCut = this.utilTimeHHmm(new Date(this.replaceTime(this.ruleObj.DayCut)))
          } else {
            Message.error('调休规则详情获取失败，请刷新重试！')
          }
        }).catch(() => {
          Message.error('调休规则详情获取失败，请刷新重试！')
        })
      },
      async _addDaysOffRule () {
        // 保存调休规则
        if (this.ruleObj.Priority === 2) {
          this.ruleObj.PSort = this.PSort
        }
        await addDaysOffRule(this.$route.query.configCode, JSON.stringify(this.ruleObj)).then(res => {
          if (res.data.State === REQ_OK) {
            Message.success('调休规则保存成功！')
            this.$router.back()
          } else {
            Message.error('调休规则保存失败，请重试！')
          }
        }).catch(() => {
          Message.error('调休规则保存失败，请重试！')
        })
