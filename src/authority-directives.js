...
import { 
    ...
    PG_PA_EEOnjob,
    PG_PA_ContractMgt,
    PG_PA_ContractRemind,
    PG_PA_BasicSet
} from '@/api/config'

Vue.directive('atris-authrity-page1-com1-btn1', {
    ...
    bind: function (el, binding, vNode) {
        ...
        switch(currentPage){
            case `${page1}`:
                let hasAuthority_page1 = authorityObj[page1]['hasAuthority']
                if(!hasAuthority_page1) {
            ...
            case `${page2}`:
                ...
                break
            case `${page3}`:
                let hasAuthority_page3 = authorityObj[page3]['com2']['btn2']['hasAuthority']
                if(!hasAuthority_page3) {
                    el.style.display = 'none'   
                }else {
...
Vue.directive('atris-authrity-coms', {
    deep: true,  // 自定义属性用在对象上，对象内部属性变化的时候触发update，在指令定义对象中指定deep:true
    bind: function (el, binding, vNode) {
        //注意：在每个函数中第一个参数，永远是el,表示被绑定了指令的那个元素，这个 el 参数，是一个原生的JS对象
        //在元素刚绑定了指令的时候，还没有插入到DOM中去，这时调用  focus 方法么有作用
