  ...
  export default {
    ...
    data(){
      return {
        ...
        formRules: {
          SysName: [{required: true, message: '请输入自定义名称', trigger: ['change','blur']}],
    ...
    methods: {
      ...
      _CompTableDataSet(){
        this.loading = true
        CompTableDataSet('' , this.queryObj.moduleCode, this.queryObj.state, this.queryObj.pageSize, this.queryObj.pageNum).then(res => {
          this.loading = false
          if(res && res.data.State === REQ_OK){
