...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      async _getPublicTripRule () {
        // 获取规则
        await getPublicTripRule(this.$route.query.ruleCode).then(res => {
          if (res.data.State === REQ_OK) {
            this.ruleObj = res.data.Data
            this.ruleObj.DayCut = this.utilTimeHHmm(new Date(this.replaceTime(this.ruleObj.DayCut)))
          } else {
            Message.error('公出规则详情获取失败，请刷新重试！')
          }
        }).catch(() => {
          Message.error('公出规则详情获取失败，请刷新重试！')
        })
      },
      async _addPublicTripRule () {
        // 保存调休规则
        if (this.ruleObj.Priority === 2) {
          this.ruleObj.PSort = this.PSort
        }
        await addPublicTripRule(this.$route.query.configCode, JSON.stringify(this.ruleObj)).then(res => {
          if (res.data.State === REQ_OK) {
            Message.success('公出规则保存成功！')
            this.$router.back()
          } else {
            Message.error('公出规则保存失败，请重试！')
          }
        }).catch(() => {
          Message.error('公出规则保存失败，请重试！')
        })
