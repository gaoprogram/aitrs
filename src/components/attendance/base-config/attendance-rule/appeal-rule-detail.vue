...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      async _getAppealRule () {
        // 获取规则
        await getAppealRule(this.$route.query.ruleCode).then(res => {
          if (res.data.State === REQ_OK) {
            this.ruleObj = res.data.Data
          } else {
            Message.error('申述规则详情获取失败，请刷新重试！')
          }
        }).catch(() => {
          Message.error('申述规则详情获取失败，请刷新重试！')
        })
      },
      async _addAppealRule () {
        await addAppealRule(this.$route.query.configCode, JSON.stringify(this.ruleObj)).then(res => {
          if (res.data.State === REQ_OK) {
            Message.success('申述规则保存成功！')
            this.$router.back()
          } else {
            Message.error('申述规则保存失败，请重试！')
          }
        }).catch(() => {
          Message.error('申述规则保存失败，请重试！')
        })
