  ...
  >
    ...
    <el-form style="width: 700px;display: inline-block"
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="失效日期" class="postInfo-container-item" prop="EndDate">
            <el-date-picker
        ...
        <el-col :span="12" v-if="isHasDefaultSetting">
          <el-form-item label="标准预设" prop="DefaultSettingId">
            <el-select class="filter-item" v-model="newSalaryTypeObj.DefaultSettingId" placeholder="请选择">
              <el-option v-for="item in DefaultSettingId" :key="item.Id" :label="item.Label" :value="item.Id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      _getTermList () {
        getNormalTermList(this.$route.query.salaryArchive).then(res => {
          if (res.data.State === REQ_OK) {
            let result = res.data.Data
            console.log(this.isRevenue)
            if (this.isRevenue) {
